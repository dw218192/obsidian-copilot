...
import { getSettings, setSettings, settingsStore } from "@/settings/model";
import {
  awaitInFlightMaterialization,
...
export class AgentSessionManager {
   ...
   * is stale). For the ACTIVE project we also warm its `.context-cache/` in the
   * background so the next chat there starts with sources already fetched — the
  ...
  /**
   * Fire-and-forget refresh of a project's `.context-cache/` snapshots. Unlike
  ...
  async getChatHistoryItems(scope?: ProjectScopeId): Promise<ChatHistoryItem[]> {
    ...
    if (scope && scope !== GLOBAL_SCOPE) {
      ...
      const scopedMarkdown = resolved.filter((entry): entry is MarkdownChatEntry => entry !== null);
      if (!index) {
  ...
  async rematerializeSource(
    ...
    // Join any in-flight full materialization (e.g. the active-project warm a
    // source edit triggers) before writing our single-source snapshot — its
    // reconcile would otherwise reap a snapshot we wrote mid-run, silently
    // reverting this Retry to failed. See awaitInFlightMaterialization.
    await awaitInFlightMaterialization(projectId);
