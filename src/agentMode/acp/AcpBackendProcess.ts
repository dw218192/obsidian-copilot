...
export class AcpBackendProcess implements BackendProcess {
  ...
  async newSession(params: OpenSessionInput): Promise<OpenSessionOutput> {
    ...
    // Forward extra roots only when the agent advertises the experimental
    // `additionalDirectories` capability (and only at session/new, never on
    // resume/load). Agents that don't advertise it get no field at all —
    // sending one they'll silently ignore would be misleading.
    if (this.supportsAdditionalDirectories() && params.additionalDirectories?.length) {
